%{?scl:%scl_package perl-Test-Warn}
%{!?scl:%global pkg_name %{name}}

Name:           %{?scl_prefix}perl-Test-Warn
Version:        0.24
Release:        9.sc1%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(Carp) >= 1.22
BuildRequires:  %{?scl_prefix}perl(constant)
BuildRequires:  %{?scl_prefix}perl(Exporter)
BuildRequires:  %{?scl_prefix}perl(ExtUtils::MakeMaker)
BuildRequires:  %{?scl_prefix}perl(File::Spec)
BuildRequires:  %{?scl_prefix}perl(strict)
BuildRequires:  %{?scl_prefix}perl(Sub::Uplevel) >= 0.12
BuildRequires:  %{?scl_prefix}perl(Test)
BuildRequires:  %{?scl_prefix}perl(Test::Builder) >= 0.13
BuildRequires:  %{?scl_prefix}perl(Test::Builder::Tester) >= 1.02
BuildRequires:  %{?scl_prefix}perl(Test::More)
BuildRequires:  %{?scl_prefix}perl(Test::Pod)
BuildRequires:  %{?scl_prefix}perl(Tree::DAG_Node) >= 1.02
BuildRequires:  %{?scl_prefix}perl(warnings)
%{?scl:%global perl_version %(scl enable %{scl} 'eval "`perl -V:version`"; echo $version')}
%{!?scl:%global perl_version %(eval "`perl -V:version`"; echo $version)}
Requires:       %{?scl_prefix}perl(:MODULE_COMPAT_%{perl_version})
Requires:       %{?scl_prefix}perl(Test::Builder) >= 0.13
Requires:       %{?scl_prefix}perl(Tree::DAG_Node) >= 1.02

# Filter under-specified dependencies
%if 0%{?rhel} && 0%{?rhel} < 7
%filter_from_requires /%{?scl_prefix}perl(Test::Builder)$/d
%filter_from_requires /%{?scl_prefix}perl(Tree::DAG_Node)$/d
%filter_setup
%else
%global __requires_exclude %{?__requires_exclude:%__requires_exclude|}^%{?scl_prefix}perl\\((Test::Builder|Tree::DAG_Node)\\)$
%endif
...
%changelog
* Tue Feb 18 2014 Petr Pisar <ppisar@redhat.com> - 0.24-9
- Specify all dependencies (bug #1066455)

* Sun Nov 24 2013 Jitka Plesnikova <jplesnik@redhat.com> - 0.24-8
- Rebuilt for SCL
